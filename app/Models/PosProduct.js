...
class PosProduct extends Model {
  ...
  async fetch_pos_product(where, select = "*") {
    ...
    let row = await Db.connection("srspos").raw(
      `SELECT ${select} FROM pos_products WHERE ${field}`,
      value
    );

    // let Q = `SELECT ${select} FROM pos_products WHERE ${field}, ${value} `;
    // console.log(Q)
  ...
  async fetch_exluded_supplier(vendor_code) {
    ...
    await Db.close();
    return row.length == 0 ? "" : row.length;
  }
}
